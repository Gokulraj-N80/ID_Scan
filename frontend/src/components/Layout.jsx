...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row font-sans">
      ...
      <aside className={`hidden md:flex flex-col ${isCollapsed ? 'w-20 p-3' : 'w-64 p-5'} bg-white border-r border-slate-200/80 sticky top-0 h-screen justify-between z-30 transition-all duration-300 ease-in-out`}>
        <div>
          {/* Company Branding Header with Collapse button */}
          <div className={`flex items-center ${isCollapsed ? 'flex-col gap-3 justify-center' : 'justify-between'} pb-6 mb-6 border-b border-slate-100`}>
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-gradient-to-tr from-indigo-600 to-indigo-500 text-white rounded-xl shadow-md shadow-indigo-500/20 shrink-0">
            ...
            <button
              ...
              className="p-1.5 hover:bg-slate-50 border border-slate-200/50 text-slate-400 hover:text-slate-700 rounded-lg transition-all shadow-3xs"
          ...
          <nav className="space-y-1.5">
            ...
            {links.map((link) => {
              ...
              return (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all ${
                    isActive
                ...
                >
                  ...
                  {!isCollapsed && isActive && <ChevronRight className="w-4 h-4 text-indigo-500" />}
                </Link>
              );
            })}
            ...
            {isCollapsed && <div className="h-px bg-slate-100 my-4" />}
        ...
        <div className="pt-4 border-t border-slate-100">
          {isCollapsed ? (
            <div className="flex flex-col items-center gap-3">
              <div 
                className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-500 to-indigo-600 text-white flex items-center justify-center font-bold text-sm shadow-sm shrink-0"
